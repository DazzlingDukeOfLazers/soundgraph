class_name GraphRack
extends Control
## The patcher, drawn as a rack rather than as a node graph.
##
## A copy of Rack, taken deliberately rather than shared: this is where the patcher goes
## from here, and the two are about to diverge. Rack stays as it is — a case of modules
## flowed into rows by the layering, with no coordinates, because a real case has none.
## GraphRack keeps the panel drawing and gains what a patcher needs that a case does not:
## free placement, cables between arbitrary modules, and a canvas the reader can pan.
##
## Why it is a copy and not a subclass: almost every method here draws, and drawing is
## exactly the part that will change. A base class factored out before knowing which half
## moves would be a guess, and the wrong guess is more expensive than the duplication.
## When the two have settled, whatever is genuinely common comes back out.
##
## Replaces the GraphEdit-based view, which is preserved at the `graph-refactor` tag.
## That view was a Godot GraphNode container: slot rows decided where ports could sit,
## the titlebar was an HBox, and every piece of the module look had to be negotiated with
...
class CableLayer extends Control:
	...
	func _draw() -> void:
		...
		for index in cables.size():
			...
			var points: PackedVector2Array
			if rack.cable_style == GraphRack.CableStyle.CATENARY:
				var span := absf(b.x - a.x)
				var sag := clampf(span * GraphRack.SAG_FRACTION, GraphRack.SAG_MIN, GraphRack.SAG_MAX)
				points = GraphRack.catenary(a, b, sag)
			else:
				var lane := maxf(a.y, b.y) + 34.0 + index * lane_step
				points = GraphRack.pcb_route(a, b, lane)
...
class RackModule extends Control:
	...
	func build(node: Dictionary) -> Dictionary:
		...
		frame.add_theme_constant_override("margin_left", int(GraphRack.KNOB_PAD))
		frame.add_theme_constant_override("margin_right", int(GraphRack.KNOB_PAD))
		frame.add_theme_constant_override("margin_top", int(GraphRack.TITLE_BAND))
		frame.add_theme_constant_override("margin_bottom", int(GraphRack.KNOB_PAD) + 8)
	...
	func _draw_analysis() -> void:
		if GraphRack.density != GraphRack.Density.ANALYSIS or rack == null:
			return
		...
		var bottom: float = size.y - GraphRack.KNOB_PAD - 10.0
		if bottom - top < 24.0:
			return
		var area := Rect2(GraphRack.KNOB_PAD + 4.0, top, size.x - GraphRack.KNOB_PAD * 2.0 - 8.0,
			bottom - top)

		# Recessed into the panel, the way a display on a real module is.
		draw_rect(area, GraphRack.JACK_HOLE)
		draw_rect(area, GraphRack.PANEL_EDGE, false, 1.0)
		...
		draw_line(Vector2(area.position.x, middle),
			Vector2(area.end.x, middle), GraphRack.KNOB_TRACK, 1.0)
	...
	func _draw() -> void:
		...
		var tint: Color = GraphRack.category_tint(str(descriptor.get("category", "")))

		# Panel, with a faint vertical gradient. Aluminium is not flat.
		draw_rect(Rect2(Vector2.ZERO, size), GraphRack.PANEL)
		var band := size.y / 8.0
		for i in 8:
			var shade: Color = GraphRack.PANEL.lerp(GraphRack.PANEL_LOW, i / 7.0)
			draw_rect(Rect2(0.0, i * band, size.x, band + 1.0), shade)

		draw_line(Vector2(0.5, 0.0), Vector2(0.5, size.y), GraphRack.PANEL_EDGE, 1.0)
		...
		draw_rect(Rect2(10.0, GraphRack.TITLE_BAND - 7.0, size.x - 20.0, 2.0),
		...
		if rack != null and rack.selected_id == node_id:
			draw_rect(Rect2(Vector2.ZERO, size), GraphRack.SELECTED, false, 2.0)
...
class Jack extends Control:
	...
	func _text_width() -> float:
		...
		return minf(font.get_string_size(port_name, HORIZONTAL_ALIGNMENT_LEFT, -1,
			_label_size()).x, Design.scale(GraphRack.JACK_LABEL_MAX))

	func _get_minimum_size() -> Vector2:
		return Vector2(GraphRack.jack_radius() * 2.0 + 6.0 + _text_width(),
			maxf(GraphRack.jack_radius() * 2.0 + 6.0, float(_label_size()) + 12.0))
	...
	func socket_centre() -> Vector2:
		return Vector2(GraphRack.jack_radius() if is_input else size.x - GraphRack.jack_radius(),
			size.y * 0.5)
	...
	func _draw() -> void:
		...
		draw_circle(centre, GraphRack.jack_radius(), Color(0.20, 0.21, 0.24))
		draw_circle(centre, GraphRack.jack_radius(), Color(0, 0, 0, 0.55), false, 1.0)
		draw_circle(centre, GraphRack.jack_radius() - 3.0, GraphRack.JACK_HOLE)
		if is_input:
			draw_circle(centre, GraphRack.jack_radius() - 1.5, colour, false, 2.0)
		else:
			draw_circle(centre, GraphRack.jack_radius() - 5.5, colour)

		var font := _label_font()
		if font == null:
			return
		var room := size.x - GraphRack.jack_radius() * 2.0 - 6.0
		if room <= 4.0:
			return
		var text := GraphRack.elided(font, port_name, _label_size(), room)
		var baseline := size.y * 0.5 + float(_label_size()) * 0.36
		draw_string(font,
			Vector2(GraphRack.jack_radius() * 2.0 + 6.0 if is_input else 0.0, baseline), text,
...
class Knob extends Control:
	...
	func _value_text() -> String:
		...
		return GraphRack.format_value(value())
		...
		if descriptor.has("enum"):
			...
			for option in descriptor["enum"]:
				if str(option).length() > longest.length():
					longest = str(option)
			return longest
		var low := GraphRack.format_value(float(descriptor.get("min", 0.0)))
		var high := GraphRack.format_value(float(descriptor.get("max", 1.0)))
	...
	func _get_minimum_size() -> Vector2:
		if compact:
			...
			var dial := GraphRack.knob_radius() * 2.0 + 8.0
	...
	func _draw() -> void:
		...
		var centre := Vector2(size.x * 0.5,
			size.y * 0.5 if compact else GraphRack.knob_radius() + 6.0)
		var angle := START + SWEEP * _position

		draw_arc(centre, GraphRack.knob_radius() + 5.0, START, START + SWEEP, 40,
			GraphRack.KNOB_TRACK, 3.0, true)
		draw_arc(centre, GraphRack.knob_radius() + 5.0, START, angle, 40,
			GraphRack.SELECTED, 3.0, true)
		...
		draw_circle(centre - Vector2(0, 1), GraphRack.knob_radius() - 5.0,
			GraphRack.KNOB_BODY.lightened(0.10))
		# The pointer, which is what actually tells you where the knob is set.
		draw_line(centre + Vector2(cos(angle), sin(angle)) * 6.0,
			centre + Vector2(cos(angle), sin(angle)) * (GraphRack.knob_radius() - 3.0),
			rack.ink, 2.5, true)
		...
		var label_font: Font = Design.font(Design.WEIGHT_MEDIUM)
		var label_size := Design.type(Design.SIZE_SECONDARY)
		var room := size.x - GraphRack.KNOB_PAD * 2.0
		var value_font: Font = Design.numeric_font()
		var value_size := Design.type(Design.SIZE_NUMERIC)
		var value_baseline := size.y - GraphRack.KNOB_PAD * 0.5
		var name_baseline := value_baseline - float(value_size) - 4.0
		draw_string(label_font, Vector2(GraphRack.KNOB_PAD, name_baseline),
			GraphRack.elided(label_font, _name_text(), label_size, room),
			HORIZONTAL_ALIGNMENT_CENTER, room, label_size, rack.ink_dim)
		draw_string(value_font, Vector2(GraphRack.KNOB_PAD, value_baseline),
			GraphRack.elided(value_font, _value_text(), value_size, room),
