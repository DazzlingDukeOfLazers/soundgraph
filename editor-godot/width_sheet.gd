...
## Everything that would make a width wrong, as a list of complaints. Empty means the node
## really is laid out correctly at the width it was given.
func complaints(widget: GraphNode, forced: float, tall: float) -> Array:
	var bad: Array = []
	...
	for scale in Design.SCALE_FACTORS.size():
		...
		for index in NodeGrid.WIDTHS.size():
			...
			for i in placeable.size():
				...
				await settle(6)
				var bad := complaints(widget, forced, tall)
				if bad.is_empty():
					settled[type] = index
