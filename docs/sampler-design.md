...
Stage 2 landed the same day: the frequency input repitches against root (exact
arithmetic, well inside the within-a-cent criterion), the slice input reads a 0-to-1
position on the gate edge and holds it for the hit, start and length trim within the
slice, and loop wraps the slice rather than the buffer. The shipped chop patch is
`examples/patches/break-chopper.json`, generated by `tools/make-break.mjs` from one bar
of the Euclid demo's own groove — the "make your own break" pipeline, with a --check in
the suite so neither the break nor its source can drift.
